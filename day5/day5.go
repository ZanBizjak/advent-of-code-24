...
func Task2() {
	...
	res := 0
	for _, update := range updates {
		if inOrder(rules, update) == 0 {
			res += findOrder(rules, update)
		}
...
func findOrder(rules map[int]map[int]bool, update string) int {
	updateArr := strings.Split(update, ",")
	slices.Reverse(updateArr)
	var updateInt []int
	for _, num := range updateArr {
	...
	updateInt = findOrderRec(rules, updateInt)

	return updateInt[len(updateInt)/2]

}

func findOrderRec(rules map[int]map[int]bool, updateInt []int) []int {
	for i := 0; i < len(updateInt)-1; i++ {
		page := updateInt[i]
		for j := i + 1; j < len(updateInt); j++ {
			if rules[page][updateInt[j]] {
				switcheroo := updateInt[j]
				updateInt[j] = updateInt[i]
				updateInt[i] = switcheroo
				return findOrderRec(rules, updateInt)
			}
